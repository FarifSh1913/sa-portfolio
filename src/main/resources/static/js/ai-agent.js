(() => {
    ...
    const lanaStage = $('aiLanaStage'), lanaStatus = $('aiLanaStatus'), history = $('aiMessageHistory'), skipIntroButton = $('aiSkipIntro');
    const chatForm = $('aiChatForm'), chatText = $('aiChatText'), reducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)');
    ...
    function addChatMessage(message, forceScroll = false) {
        ...
        // Existing messageHistory.scrollTo behavior is kept through the history alias.
        const row = document.createElement('article'); row.className = `ai-message-row${message.sender === 'nikolay' ? ' is-nikolay' : ''}`; row.dataset.sender = message.sender;
    ...
    function responseStartedApproval(response) { return Boolean(extractProcessKey(response) && response.approvalStarted !== false); }
    ...
    async function sendChatMessage() {
        ...
        try {
            ...
            if (responseStartedApproval(response)) startApproval(response);
    ...
    function renderWaiting(text) {
        assistant.classList.add('ai-state-screen'); assistant.querySelector('.ai-lana-character').hidden = true; assistant.querySelector('.ai-chat-header').hidden = true; history.className = 'ai-state-content';
        history.innerHTML = `<div class="ai-status-card"><span class="ai-loader" aria-hidden="true"></span><h3>Вариант отправлен Тамаре Геннадьевне</h3><p>${text.replace(/\n/g, '<br>')}</p></div>`;
    }
    ...
    function renderCompleted(approved, comment) {
        pollController?.abort(); const content = assistant.querySelector('.ai-state-content'); content.replaceChildren(); const card = document.createElement('div'); card.className = 'ai-status-card';
        card.innerHTML = `<h3>${approved ? 'Согласовано' : 'Отказано в согласовании'}</h3><p>${approved ? 'Тамара Геннадьевна согласовала восстановление забора.' : (comment ? `Комментарий Тамары Геннадьевны: ${escapeHtml(comment)}` : 'Тамара Геннадьевна отказала в согласовании.')}</p><button type="button" class="btn btn-dark" id="aiRestartScenario">Начать сценарий заново</button>`; content.append(card); $('aiRestartScenario').addEventListener('click', restartScenario);
    }
    ...
    function finishIntro() {
        ...
        skipIntroButton.hidden = true; setState(STATES.CHAT); setChatEnabled(true); chatText.focus();
    }
    function skipIntro() {
        if (state !== STATES.INTRO) return;
        for (let index = introMessagesShown; index < STORY_MESSAGES.length; index += 1) {
            addChatMessage(STORY_MESSAGES[index], true); introMessagesShown = index + 1;
        }
        finishIntro();
    }
